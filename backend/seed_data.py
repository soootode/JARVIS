...
(id=1) می‌نوشت که همه‌ی کاربران سیستم آن را "می‌دیدند" — یعنی پروفایل
روان‌شناختی شخصی خودت (ستوده) برای هر کسی که signup می‌کرد اعمال می‌شد.
الان یک user_id مشخص می‌گیرد (آرگومان CLI یا env) و فقط پروفایل همان یک
حساب را می‌سازد/به‌روزرسانی می‌کند. برای onboarding خودکار کاربران جدید
از این اسکریپت استفاده نکن — آن باید از طریق ویزارد /api/onboarding
(بخش فرانت‌اند) با داده‌ای که خودِ کاربر وارد می‌کند انجام شود. این فایل
فقط برای seed کردن حساب شخصی خودت (یا داده‌ی دمو) است:

    python seed_data.py --email you@example.com
"""
...
<<<<<<< HEAD
        ...
        if not user:
            ...
            print(f"     اول باید از /api/auth/signup ثبت‌نام کنی، بعد این اسکریپت را اجرا کن.")
            return

        seed_persona(db, user.id)
        seed_psychology(db, user.id)
        seed_goals(db, user.id)
        seed_habits(db, user.id)

        # onboarding را برای این حساب تکمیل‌شده علامت بزن
...
<<<<<<< HEAD
    ...
    parser.add_argument(
        "--email", required=True,
        help="ایمیل حسابی که قبلاً از طریق /api/auth/signup ساخته شده",
    )
